...
function getPlatformDisplayName(platform) {
  ...
  return platform.isSearchFallback ? `${baseName} Search` : baseName;
}
...
export function TitleDetail() {
  ...
  if (isLoading) {
    return (
      <div className="detail-loading">
        <div className="detail-loading-spinner">Loading</div>
  ...
  if (!title) {
    return (
      <div className="detail-not-found">
        <span className="not-found-icon">Not Found</span>
  ...
  const displaySynopsis = shouldTruncate && !showFullSynopsis
    ? `${rawSynopsis.slice(0, 300)}...`
    : rawSynopsis;
  const primaryTitle = title.title_th || title.title_en;
  const secondaryTitleParts = [title.title_en, title.title_native]
    .filter(Boolean)
    .filter((entry, index, list) => list.indexOf(entry) === index)
    .filter((entry) => entry !== primaryTitle);
  const secondaryTitle = secondaryTitleParts.join(' / ');
  const synopsisHeading = language === 'th' ? 'เรื่องย่อ' : 'Synopsis';
  ...
  const handleToggleTopTitle = async () => {
    try {
      if (inTopTitles) {
        ...
        toast.success('นำออกจาก Top 5 แล้ว');
        return;
      }

      const result = await addToTopTitles(title);
      if (!result.ok && result.reason === 'limit') {
        toast.error(`Top ${limit} ของ ${title.type} เต็มแล้ว`);
        return;
      }

      toast.success(`เพิ่มเข้า Top 5 ${title.type} แล้ว`);
    } catch (error) {
      console.error(error);
      toast.error('อัปเดต Top 5 ไม่สำเร็จ');
    }
  };
  ...
  const handleSubmitReport = async (event) => {
    ...
    if (!supabase) {
      toast.error('Unable to connect to reporting service');
      return;
    }
      ...
      <div className="container relative z-10 detail-content">
        <div className="detail-header">
          <div className="cover-wrapper animate-scale-in">
            <img src={title.cover} alt={title.title_en} className="detail-cover" />
            ...
            <div className="badges-row">
              ...
              <div className="detail-title-row">
                ...
                <button
                  ...
                  aria-label={showReportForm ? 'Hide report form' : 'Open report form'}
                  title={showReportForm ? 'Hide report form' : 'Report issue'}
                >
                  <Flag size={15} />
                </button>
              </div>
	            {secondaryTitle && (
	              <h2 className="title-secondary">{secondaryTitle}</h2>
	            )}
              ...
              <button
                className={`top-title-chip ${inTopTitles ? 'active' : ''}`}
                onClick={handleToggleTopTitle}
              >
                <Trophy size={18} fill={inTopTitles ? 'currentColor' : 'none'} />
                <span>{inTopTitles ? `Top ${topTitleRank} ${title.type}` : `Add to Top 5 ${title.type}`}</span>
              </button>
                ...
                {status ? (
                  <>
                    <div className="status-selector-wrapper">
                      ...
                      <button className="remove-btn" onClick={() => removeFromList(title.id)}>
                      ...
                      <div className="progress-editor">
                        ...
                        <label className="progress-field">
                          ...
                          <input
                            ...
                            onChange={(event) => setConsumptionTarget(
                              title,
                  ...
                  <div className="add-actions">
                    <Button onClick={() => addToList(title.id, 'planned')}>
                      {t('titleDetail.addToList')}
                    </Button>
                    <Button variant="secondary" onClick={() => addToList(title.id, 'completed')}>
                      {t('titleDetail.markCompleted')}
                    </Button>
	                  ...
	                  <div>
	                    ...
	                    {orderedPlatforms.length > 0 && (
	                      <div className="platform-caption">
	                        {orderedPlatforms.length} link{orderedPlatforms.length === 1 ? '' : 's'} available
	                      </div>
	                    )}
	                  </div>
	                ...
	                {orderedPlatforms.length > 0 ? (
	                  <div className="platform-groups">
	                    ...
	                    {fallbackPlatforms.length > 0 && (
	                      <div className="platform-group">
	                        <span className="platform-group-label">Search</span>
	                        <div className="platforms">
	                          {fallbackPlatforms.map((platform) => (
	                            <a key={`${platform.name}-${platform.url}`} href={platform.url} target="_blank" rel="noreferrer" className="platform-link platform-link-fallback">
	                              <span className="platform-link-name">{platform.displayName}</span>
	                              <span className="platform-link-tag">Search</span>
	                            </a>
	                          ))}
	                        </div>
	                      </div>
	                  ...
	                  <div className="platforms-empty-state">
	                    <span className="platforms-empty-icon"><Link size={15} /></span>
                ...
                {showReportForm && (
	              <section className="report-panel">
                    ...
                    <label className="report-field report-field-wide">
                      <span>What should staff check?</span>
                      <textarea
                        value={reportForm.description}
                        onChange={(event) => setReportForm((current) => ({ ...current, description: event.target.value }))}
                        placeholder="Add context, the wrong field, or the broken platform link."
                        rows={4}
                      />
                    </label>
                    <div className="report-actions">
                      <span className="report-helper">{user ? 'Submitted reports enter the moderation queue immediately.' : 'Sign in is required to submit a report.'}</span>
        ...
        {rawSynopsis && (
          <section className="detail-synopsis-panel">
            <div className="synopsis-wrapper">
              <h3 className="detail-section-heading">{synopsisHeading}</h3>
              ...
              {shouldTruncate && (
                <button className="synopsis-toggle" onClick={() => setShowFullSynopsis((current) => !current)}>
            ...
            <div className="similar-header">
              ...
              <div className="similar-nav">
                <button
                  ...
                  aria-label="Previous similar titles"
                >
                  <ChevronLeft size={18} />
                </button>
                <button
                  ...
                  aria-label="Next similar titles"
                >
