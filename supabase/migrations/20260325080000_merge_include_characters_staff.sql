-- Patch admin_merge_duplicate_titles to migrate title_characters and title_staff
-- before the duplicate canonical_title is deleted (which would cascade-delete them).
-- Also fills in episode_count / chapter_count from the duplicate when the primary has none.
...
begin
  ...
  -- characters: move all from duplicate to primary.
  -- Skip ones where the same anilist_id already exists on the primary.
  ...
  -- Delete any remaining characters that were duplicated (same anilist_id already on primary)
  ...
  -- other duplicate_candidates referencing the merged title
