create table if not exists public.battle_sessions (
  ...
  target_rounds integer not null default 24 check (target_rounds >= 0),
  ...
  ranking jsonb,
  tiers jsonb,
  winner_title_id bigint references public.canonical_titles(id) on delete set null,
...
create index if not exists idx_battle_sessions_user_updated_at
on public.battle_sessions(user_id, updated_at desc);
...
create index if not exists idx_battle_sessions_user_status_updated_at
on public.battle_sessions(user_id, status, updated_at desc);

drop trigger if exists trg_battle_sessions_updated_at on public.battle_sessions;
create trigger trg_battle_sessions_updated_at
...
create table if not exists public.battle_votes (
  ...
  left_title_id bigint not null references public.canonical_titles(id) on delete cascade,
  right_title_id bigint not null references public.canonical_titles(id) on delete cascade,
...
create index if not exists idx_battle_votes_session_sequence
on public.battle_votes(session_id, sequence_index asc);

create index if not exists idx_battle_votes_user_created_at
on public.battle_votes(user_id, created_at desc);

create index if not exists idx_battle_votes_left_title_created_at
on public.battle_votes(left_title_id, created_at desc);

create index if not exists idx_battle_votes_right_title_created_at
on public.battle_votes(right_title_id, created_at desc);
