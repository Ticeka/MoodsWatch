create table if not exists public.duplicate_candidates (
  ...
  title_a_id bigint references public.canonical_titles(id) on delete set null,
  title_b_id bigint references public.canonical_titles(id) on delete set null,
  suggested_primary_title_id bigint references public.canonical_titles(id) on delete set null,
  ...
  unique (title_a_id, title_b_id),
  check (title_a_id is null or title_b_id is null or title_a_id < title_b_id)
);

create table if not exists public.duplicate_merge_actions (
  id bigint generated by default as identity primary key,
  candidate_id bigint references public.duplicate_candidates(id) on delete set null,
  primary_title_id bigint,
  duplicate_title_id bigint,
...
create index if not exists idx_duplicate_candidates_status_confidence
on public.duplicate_candidates(status, confidence desc, created_at desc);

create index if not exists idx_duplicate_candidates_primary
on public.duplicate_candidates(suggested_primary_title_id, created_at desc);

create index if not exists idx_duplicate_merge_actions_created
on public.duplicate_merge_actions(created_at desc);
...
alter table if exists public.duplicate_candidates enable row level security;
alter table if exists public.duplicate_merge_actions enable row level security;

drop policy if exists "Staff can manage duplicate candidates" on public.duplicate_candidates;
create policy "Staff can manage duplicate candidates"
on public.duplicate_candidates for all
using (public.is_staff_user())
with check (public.is_staff_user());

drop policy if exists "Staff can read duplicate merge actions" on public.duplicate_merge_actions;
create policy "Staff can read duplicate merge actions"
on public.duplicate_merge_actions for select
using (public.is_staff_user());

drop policy if exists "Staff can insert duplicate merge actions" on public.duplicate_merge_actions;
create policy "Staff can insert duplicate merge actions"
on public.duplicate_merge_actions for insert
with check (public.is_staff_user());

create or replace function public.replace_title_ids_in_jsonb_array(input jsonb, keep_id bigint, drop_id bigint)
returns jsonb
language sql
immutable
as $$
  with expanded as (
    select
      case
        when value::bigint = drop_id then keep_id
        else value::bigint
      end as title_id,
      ordinality
    from jsonb_array_elements_text(coalesce(input, '[]'::jsonb)) with ordinality as t(value, ordinality)
  ),
  dedup as (
    select distinct on (title_id) title_id, ordinality
    from expanded
    order by title_id, ordinality
  )
  select coalesce(jsonb_agg(to_jsonb(title_id) order by ordinality), '[]'::jsonb)
  from dedup;
$$;

create or replace function public.replace_top_titles_payload(input jsonb, keep_id bigint, drop_id bigint)
returns jsonb
language sql
immutable
as $$
  select jsonb_build_object(
    'anime', public.replace_title_ids_in_jsonb_array(input->'anime', keep_id, drop_id),
    'manga', public.replace_title_ids_in_jsonb_array(input->'manga', keep_id, drop_id),
    'manhwa', public.replace_title_ids_in_jsonb_array(input->'manhwa', keep_id, drop_id)
  );
$$;
...
begin
  ...
  set
    title_a_id = case when title_a_id = p_duplicate_title_id then p_primary_title_id else title_a_id end,
    title_b_id = case when title_b_id = p_duplicate_title_id then p_primary_title_id else title_b_id end,
