-- Fix: duplicate_candidates check constraint violation during merge.
-- When a row has (title_a_id = duplicate, title_b_id = primary) or vice versa,
-- the UPDATE would set both sides to primary_id → title_a_id = title_b_id → violates
-- the check (title_a_id < title_b_id). Delete those rows first, then update the rest.
...
begin
  ...
  -- characters: move all from duplicate to primary, skip anilist_id already present
  update public.title_characters
  set canonical_title_id = p_primary_title_id
  where canonical_title_id = p_duplicate_title_id
    and (
      anilist_id is null
      or anilist_id not in (
        select anilist_id
        from public.title_characters
        where canonical_title_id = p_primary_title_id
          and anilist_id is not null
      )
    );

  delete from public.title_characters
  where canonical_title_id = p_duplicate_title_id;

  -- staff: same approach
  update public.title_staff
  set canonical_title_id = p_primary_title_id
  where canonical_title_id = p_duplicate_title_id
    and (
      anilist_id is null
      or anilist_id not in (
        select anilist_id
        from public.title_staff
        where canonical_title_id = p_primary_title_id
          and anilist_id is not null
      )
    );

  delete from public.title_staff
  where canonical_title_id = p_duplicate_title_id;
  ...
  -- duplicate_candidates: delete rows that would become self-referential FIRST,
  -- then update remaining references safely.
  delete from public.duplicate_candidates
  where id <> p_candidate_id
    and (
      (title_a_id = p_duplicate_title_id and title_b_id = p_primary_title_id)
      or
      (title_a_id = p_primary_title_id and title_b_id = p_duplicate_title_id)
    );
  ...
  set
    ...
    suggested_primary_title_id = case
      when suggested_primary_title_id = p_duplicate_title_id then p_primary_title_id
      else suggested_primary_title_id
    end
  where id <> p_candidate_id
    and (
      title_a_id = p_duplicate_title_id
      or title_b_id = p_duplicate_title_id
      or suggested_primary_title_id = p_duplicate_title_id
    );

  -- also patch suggested_primary_title_id-only rows without touching title_a/b ordering
  update public.duplicate_candidates
